...
c2) Fit, including errors and associated errors
for the parameters a_i
'''
...
# No error fit to obtain the first estimate of the slope
alpha, beta, sigma_alpha, sigma_beta = linearFit(logM,logsigma, np.ones(n), np.ones(n))

# Fit including errors sigma_y and sigma_x
alpha, beta, sigma_alpha, sigma_beta = \
linearFit(logM,logsigma, e_logM, e_logsigma, beta)
...
plt.legend()
plt.savefig('M-sigma_with_errors.pdf')
plt.show()
